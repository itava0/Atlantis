...
export default class CommunalSpaceManager extends LightningElement {
  ...
  @wire(getRecord, {
    recordId: "$recordId",
    fields: [IN_COMMUNITY, COMMUNITY_ID]
  })
  ...
  handleJoin(event) {
    updateCommunity({
      recordId: this.recordId,
      communityId: event.target.dataset.id
    }).then(() => {
      this.inCommunity = true;
  ...
  handleNew() {
    ...
    const recordInput = {
      apiName: COMMUNAL_SPACE_OBJECT.objectApiName,
      fields
    };
    createRecord(recordInput).then((communalSpace) => {
