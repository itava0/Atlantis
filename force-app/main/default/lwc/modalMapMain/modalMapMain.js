...
export default class ModalMapMain extends LightningModal {
    ...
    hotelSelected = false; restaurantSelected = false; airportSelected = false; schoolSelected = false; gymSelected = false;
    bankSelected = false; hospitalSelected = false; supermarketSelected = false; theaterSelected = false; librarySelected = false;
    hotelDisabled = false; restaurantDisabled = false; airportDisabled = false; schoolDisabled = false; gymDisabled = false;
    bankDisabled = false; hospitalDisabled = false; supermarketDisabled = false; theaterDisabled = false; libraryDisabled = false;
    noneSelected = true;

    // Determines how to display buttons for each type
    hotelVariant = 'brand-outline'; restaurantVariant = 'brand-outline'; airportVariant = 'brand-outline'; schoolVariant = 'brand-outline';
    gymVariant = 'brand-outline'; bankVariant = 'brand-outline'; hospitalVariant = 'brand-outline'; supermarketVariant = 'brand-outline';
    theaterVariant = 'brand-outline'; libraryVariant = 'brand-outline';

    // Checking if a type has already been searched, to prevent repeat API calls
    hotelSearched = false; restaurantSearched = false; airportSearched = false; schoolSearched = false; gymSearched = false;
    bankSearched = false; hospitalSearched = false; supermarketSearched = false; theaterSearched = false; librarySearched = false;
    searchedNum = 0;
    ...
    handleReset() {
        ...
        this.hotelSelected = false; this.restaurantSelected = false; this.airportSelected = false; this.schoolSelected = false; this.gymSelected = false;
        this.bankSelected = false; this.hospitalSelected = false; this.supermarketSelected = false; this.theaterSelected = false; this.librarySelected = false;
        this.hotelDisabled = false; this.restaurantDisabled = false; this.airportDisabled = false; this.schoolDisabled = false; this.gymDisabled = false;
        this.bankDisabled = false; this.hospitalDisabled = false; this.supermarketDisabled = false; this.theaterDisabled = false; this.libraryDisabled = false;
        this.hotelVariant = 'brand-outline'; this.restaurantVariant = 'brand-outline'; this.airportVariant = 'brand-outline'; this.schoolVariant = 'brand-outline';
        this.gymVariant = 'brand-outline'; this.bankVariant = 'brand-outline'; this.hospitalVariant = 'brand-outline'; this.supermarketVariant = 'brand-outline';
        this.theaterVariant = 'brand-outline'; this.libraryVariant = 'brand-outline';
    }
    ...
    getNearbyPlaces() {
        ...
        for (let i = 0; i < this.types.length; i++) {
            ...
            .then((result) => {
                ...
                } else if (this.types[i] == 'movie_theater' && !this.theaterSearched) {
                    this.theaters = result;
                    this.theaterSearched = true;
                } else if (this.types[i] == 'library' && !this.librarySearched) {
                    this.libraries = result;
                    this.librarySearched = true;
                }
                ...
                if (this.theaterSearched && this.theaterSelected) this.searchedNum++;
                if (this.librarySearched && this.librarySelected) this.searchedNum++;
            ...
            .catch((error) => {
                ...
                this.hotels = undefined; this.restaurants = undefined; this.airports = undefined; this.schools = undefined; this.gyms = undefined;
                this.banks = undefined; this.hospitals = undefined; this.supermarkets = undefined; this.theaters = undefined; this.libraries = undefined;
            });
        }
    }
    ...
    updateSelected() {
        ...
        this.theaterSelected = this.types.includes('movie_theater') ? true : false;
        this.librarySelected = this.types.includes('library') ? true : false;

        // Updates button variants based on current selection
        this.hotelVariant = this.hotelSelected ? 'brand' : 'brand-outline';
        this.restaurantVariant = this.restaurantSelected ? 'brand' : 'brand-outline';
        this.airportVariant = this.airportSelected ? 'brand' : 'brand-outline';
        this.schoolVariant = this.schoolSelected ? 'brand' : 'brand-outline';
        this.gymVariant = this.gymSelected ? 'brand' : 'brand-outline';
        this.bankVariant = this.bankSelected ? 'brand' : 'brand-outline';
        this.hospitalVariant = this.hospitalSelected ? 'brand' : 'brand-outline';
        this.supermarketVariant = this.supermarketSelected ? 'brand' : 'brand-outline';
        this.theaterVariant = this.theaterSelected ? 'brand' : 'brand-outline';
        this.libraryVariant = this.librarySelected ? 'brand' : 'brand-outline';
        ...
        if (this.types.length == TYPE_LIMIT) {
            ...
            this.theaterDisabled = this.theaterSelected ? false : true;
            this.libraryDisabled = this.librarySelected ? false : true;
    ...
    // Populate markers for a given search parameter
    populatePlaceMarkers(results, selected, icon, labelSingular, labelPlural) {
        if (results && selected && results.length > 0) {
            let amount = 0;
            for (let i = 0; i < results.length; i++) {
                if (results[i].ratingNum > this.minRating && results[i].distance < this.maxDistance) {
                    amount++;
                    this.mapMarkers.push({
                        location: {
            ...
            // Determine label display for results text
            if (!this.firstFound) this.resultText += 'Found: ';
            else this.resultText += ', ';
            this.firstFound = true;
            this.resultText += amount + ' ';
            if (amount == 1) this.resultText += labelSingular;
            else this.resultText += labelPlural;
        }
    }
    ...
    populateMarkers() {
        ...
        // For each property type, populate markers if type has been selected
        this.firstFound = false;
        this.resultText = '';
        this.populatePlaceMarkers(this.hotels, this.hotelSelected, hotelIcon, 'hotel', 'hotels');
        this.populatePlaceMarkers(this.restaurants, this.restaurantSelected, restaurantIcon, 'restaurant', 'restaurants');
        this.populatePlaceMarkers(this.airports, this.airportSelected, airportIcon, 'airport', 'airports');
        this.populatePlaceMarkers(this.schools, this.schoolSelected, schoolIcon, 'school', 'schools');
        this.populatePlaceMarkers(this.gyms, this.gymSelected, gymIcon, 'gym', 'gyms');
        this.populatePlaceMarkers(this.banks, this.bankSelected, bankIcon, 'bank', 'banks');
        this.populatePlaceMarkers(this.hospitals, this.hospitalSelected, hospitalIcon, 'hospital', 'hospitals');
        this.populatePlaceMarkers(this.supermarkets, this.supermarketSelected, supermarketIcon, 'supermarket', 'supermarkets');
        this.populatePlaceMarkers(this.theaters, this.theaterSelected, theaterIcon, 'theater', 'theaters');
        this.populatePlaceMarkers(this.libraries, this.librarySelected, libraryIcon, 'library', 'libraries');
    }

    // When marker selected
    handleMarkerSelect(event) {
        this.selectedMarkerValue = event.target.selectedMarkerValue;
        if (this.noMarkerSelected) this.noMarkerSelected = false;
    }
